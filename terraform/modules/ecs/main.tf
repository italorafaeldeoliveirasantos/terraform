...
resource "aws_alb_target_group" "alb_target_group" {
  name     = "${var.service_name}-alb-target-group-${random_id.target_group_sufix.hex}"
  port     = 80
  protocol = "HTTP"
  vpc_id   = "${var.vpc_id}"
  target_type = "ip"
...
resource "aws_security_group" "web_inbound_sg" {
  name        = "${var.service_name}-web-inbound-sg"
...
resource "aws_alb" "alb_nginx" {
  name            = "${var.service_name}-alb-nginx"
...
resource "aws_iam_role" "ecs_autoscale_role" {
  name               = "${var.service_name}_ecs_autoscale_role"
...
resource "aws_cloudwatch_metric_alarm" "service_cpu_high" {
  alarm_name          = "${var.service_name}_nginx_web_cpu_utilization_high"
